...
pub fn batch_add_favorites(favorites: &[AnimeResult], profile_id: Option<&str>) -> AppResult<()> {
    ...
    with_db(|conn| {
        ...
        for chunk in favorites.chunks(chunk_size) {
            ...
            let mut target_pids: Vec<String> = Vec::with_capacity(chunk.len());
            let mut status_vals: Vec<String> = Vec::with_capacity(chunk.len());

            for fav in chunk {
                target_pids.push(fav.profile_id.as_deref().unwrap_or(default_target).to_string());
                status_vals.push(fav.status.as_deref().unwrap_or("favorite").to_string());
            }
